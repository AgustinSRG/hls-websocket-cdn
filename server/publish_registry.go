...
// Announces to the publish database that this server
// has a stream with [streamId] being published
// This method must be called periodically, as the value is temporal
func (pr *RedisPublishRegistry) AnnouncePublishedStream(streamId string) error {
	status := pr.redisClient.Set(context.Background(), streamId, pr.config.ExternalWebsocketUrl, time.Duration(pr.config.PublishRefreshIntervalSeconds)*2*time.Second)
	return status.Err()
}
